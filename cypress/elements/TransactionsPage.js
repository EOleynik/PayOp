...
class TransactionsPage {
    ...
    checkAmountUIRUB(payAmount) {
        ...
        cy.request({
            ...
            let fixcom = response.body.data[7].value.RUB[0];
            let perscom = response.body.data[7].value.RUB[1];
            ...
            let comissionsSum = (+fixcom + (+payAmount / 100 * +perscom)).toFixed(2);


            // логика для стратегии All и разбивок для этой стратегии. Если стратегия 1 (All) и часть плательщика в оплате комиссий = 0
            if (strateg === 1) {

                if (payerPart === 0) {

                    // отнимаем сумму комисий от стоимости товара
                    let rezult = (payAmount - comissionsSum).toFixed(2);

                    cy.log("стратегия комиссий =" + " " + strateg);
                    cy.log("цена товара =" + " " + payAmount);
                    cy.log("сумма комиссий =" + " " + comissionsSum);
                ...
                } else { // логика для стратегии All и разбивок для этой стратегии.
                    // Если стратегия 1 (All) и часть плательщика в оплате комиссий не = 0, то сумму коммисий делим на 100 и умножаем на процент мерчанта.
                    let comissionWithoutPayerPart = (comissionsSum / 100) * userPart;

                    // отнимаем сумму комисий от стоимости товара
                    let rezult = (payAmount - comissionWithoutPayerPart).toFixed(2);
                ...
                // логика для стратегии MAX и разбивок для этой стратегии. Если стратегия 2 (MAX) и часть плательщика в оплате комиссий = 0
            } else {
                if (payerPart === 0) {
                    ...
                    if (fixcom > (+payAmount / 100 * +perscom)) {
                        // отнимаем фиксированную комиссию от стоимости товара
                        let rezult = (payAmount - (fixcom / 100 * userPart)).toFixed(2);
                        ...
                        // отнимаем процент комиссии от стоимости товара
                        let rezult = (payAmount - ((+payAmount / 100 * +perscom) / 100 * payerPart)).toFixed(2);
    ...
    checkAmountUIUSD(payAmount) {
        ...
        cy.request({
            method: 'GET',
            url: "https://app.stage.payop.com/v1/instrument-settings/commissions/custom/" + paymentMethod.pm_id + "/" + merchant.bussiness_account,
            headers: {
                token: feen.token,
            }
        }).then((response) => {
    ...
    checkAmountAPIGBP(payAmount) {
        ...
        cy.request({
            ...
            cy.request({
                method: 'GET',
                url: 'https://app.stage.payop.com/v1/transactions/user-transactions?query[type]=7',
                headers: {
    ...
    checkCreateRefund() {
        ...
        cy.request({
            method: 'GET',
            url: 'https://app.stage.payop.com/v1/transactions/user-transactions',
            headers: {
                token: merchant.token,
            }
        }).then((response) => {
            ...
            cy.request({
                method: 'GET',
                url: "https://app.stage.payop.com/v1/transactions/" + trIdent,
                headers: {
                    token: merchant.token,
                }
            }).then((response) => {
